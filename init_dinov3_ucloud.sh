...
# New Jupyter terminals source ~/.bashrc. Add one stable hook so terminals
# opened after initialization automatically use the correct Python and paths.
BASHRC_PATH="${HOME}/.bashrc"
AUTO_ACTIVATE_MARKER="# >>> AIPoweredMicroscope DINOv3 >>>"
touch "$BASHRC_PATH"
if ! grep -Fq "$AUTO_ACTIVATE_MARKER" "$BASHRC_PATH"; then
  cat >> "$BASHRC_PATH" <<EOF

$AUTO_ACTIVATE_MARKER
if [[ -f $(printf '%q' "$PROJECT_DIR/dinov3_ucloud_env.sh") ]]; then
  source $(printf '%q' "$PROJECT_DIR/dinov3_ucloud_env.sh")
fi
# <<< AIPoweredMicroscope DINOv3 <<<
EOF
fi
