...
class RFDETR:
    ...
    def train_from_config(self, config, **kwargs):
        ...
        with open(Path(config.dataset_dir) / "train" / "images" / "_annotations.coco.json", "r") as f:
            anns = json.load(f)
            num_classes = len(anns["categories"])
            class_names = [c["name"] for c in anns["categories"] if c["supercategory"] != "none"]
            self.model.class_names = class_names
            if mlflow.active_run():
                label_dir = Path(config.dataset_dir) / "labels"
                n_labels = len(list(label_dir.glob("*.txt")))
                mlflow.log_param("num_train_samples", n_labels)

        with open(Path(config.dataset_dir) / "valid" / "images" / "_annotations.coco.json", "r"):
            if mlflow.active_run():
                label_dir = Path(config.dataset_dir) / "labels"
                n_labels = len(list(label_dir.glob("*.txt")))
                mlflow.log_param("num_val_samples", n_labels)
        ...
        self.model.train(callbacks=self.callbacks, **all_kwargs)
        #stats = self.model.train(callbacks=self.callbacks, **all_kwargs)
        #return stats
