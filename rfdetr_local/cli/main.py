...
def train_from_rf_project(rf_project: roboflow.Project, dataset_version: int):
    ...
    rf_detr.train(dataset_dir=location, epochs=1, device="cuda" if device_supports_cuda else "cpu")


def train_from_coco_dir(coco_dir: str):
    rf_detr = RFDETRBase()
    rf_detr.train(dataset_dir=coco_dir, epochs=1, device="cuda" if device_supports_cuda else "cpu")
